...
function App() {
    ...
    const initReport = async () => {
      if (!reportId) {
        try {
          ...
          console.error("Error creating initial report:", e);
        }
      }
    };
    initReport();
  ...
  const handleSelectDriver = (driver) => {
    ...
    if (driver.area) setSelectedPlads(driver.area);
  ...
  const handleParseMail = async () => {
    ...
    try {
      ...
      if (res.data.success && res.data.tours.length > 0) {
        const toursToCreate = res.data.tours.map(t => ({
          ...t,
          plads: selectedPlads,
        ...
        setMailText("");
        toast.success(`${res.data.count} ture tilføjet!`);
      } else {
  ...
  const handleAddPause = async () => {
    ...
    const pauseEnd = new Date(now.getTime() + 45 * 60000).toTimeString().slice(0, 5);
    ...
    try {
      ...
      toast.success(`Pause: ${pauseStart} - ${pauseEnd} (45 min)`);
    } catch (e) {
  ...
  // Sort: on_way first, then normal, then completed
  const sortedTours = [...filteredTours].sort((a, b) => {
    ...
    if (aHaster !== bHaster) return aHaster - bHaster;
    return 0;
  });
          ...
          <div className="p-4">
            ...
            <div className="flex flex-wrap gap-3 mt-4">
              ...
              <button onClick={handleAddPause}
                className="flex items-center gap-2 px-4 py-3 bg-slate-200 dark:bg-slate-700 rounded-lg font-medium hover:bg-slate-300">
                <Pause className="w-5 h-5" /> + Pause (45 min)
              </button>
